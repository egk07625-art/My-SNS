...
- [x] PRD.md에 로그인창 UI/UX 명세 추가
- [x] TODO.md에 로그인창 개선 섹션 추가

### 1-1-2. 에러 처리 유틸리티

- [x] Clerk 에러 처리 유틸리티 생성 (`lib/utils/clerk-errors.ts`)
  - [x] ClerkError 타입별 한국어 메시지 매핑
  - [x] 네트워크 에러 감지 함수
  - [x] 에러 타입 분류 함수

### 1-1-3. 로그인 페이지 개선

- [x] 로그인 페이지 개선 (`app/sign-in/[[...sign-in]]/page.tsx`)
  - [x] Clerk Appearance 설정 적용
  - [x] 반응형 디자인 개선 (Mobile/Tablet/Desktop)
  - [x] 로딩 상태 표시
  - [x] 네트워크 상태 감지 및 표시
  - [x] 로그인 화면 백그라운드에 공개 피드 노출 (익명 조회)
  - [x] 로그인 전 상태에서 배경 블러/투명도 강화 (글자 식별 불가 수준)
  - [x] 헤더(Header) 로그인 페이지에 표시

### 1-1-4. 에러 디스플레이 컴포넌트

- [x] 에러 디스플레이 컴포넌트 생성 (`components/auth/ErrorDisplay.tsx`)
  - [x] 에러 메시지 표시 컴포넌트
  - [x] 재시도 버튼
  - [x] 네트워크 상태 표시
  - [x] 개발 환경 상세 에러 정보 표시
...
- [x] 환경 변수 검증 유틸리티 생성 (`lib/utils/env-validation.ts`)
  - [x] Clerk 환경 변수 검증
  - [x] Supabase 환경 변수 검증
  - [x] 개발 환경에서만 실행
  - [x] 경고 메시지 표시
  - [x] 앱 시작 시 자동 검증 (`app/layout.tsx`)

### 1-1-6. 로그인 후 플로우 개선

- [x] 로그인 후 플로우 개선 (`hooks/use-sync-user.ts`)
  - [x] 동기화 재시도 로직 추가 (최대 3회, 지수 백오프)
  - [x] 에러 발생 시 로그 기록 (개발 환경)
  - [x] 동기화 실패 시에도 사용자 경험 유지
  - [x] 네트워크 에러 감지 및 처리

### 1-1-7. 테스트 및 검증

- [ ] 테스트 및 검증
  - [ ] 잘못된 이메일/비밀번호 입력 시 한국어 에러 메시지 표시
  - [ ] 네트워크 오프라인 상태에서 에러 메시지 표시
  - [ ] 로그인 성공 후 리다이렉트 정상 동작
  - [ ] Supabase 동기화 실패 시에도 로그인 유지
  - [ ] 반응형 디자인 (Mobile/Tablet/Desktop) 확인
  - [ ] Appearance 설정 정상 적용 확인
  - [ ] 로딩 상태 표시 확인
  - [ ] 환경 변수 누락 시 개발 환경 경고 표시
  - [ ] 로그인 페이지 배경 블러 강도/투명도 검증 (다양한 해상도)
  - [ ] 익명 피드 조회 실패 시 스켈레톤 폴백 확인
  - [ ] 모달/페이지 렌더링 간 z-index 겹침 이슈 확인
...
- [x] PostCard에 좋아요 버튼 추가
  - [x] 좋아요 아이콘 (Heart, lucide-react)
  - [x] 빈 하트 ↔ 빨간 하트 상태 전환
  - [x] 클릭 시 좋아요/좋아요 취소
  - [x] 애니메이션 (scale(1.3) → scale(1))
  - [x] 더블탭 좋아요 (모바일, 이미지 영역)
  - [x] 더블탭 시 큰 하트 등장 애니메이션
- [x] `/api/likes` POST API (좋아요 생성)
  - [x] 인증 확인
  - [x] post_id 검증
  - [x] 중복 좋아요 방지
  - [x] 에러 처리
- [x] `/api/likes` DELETE API (좋아요 취소)
  - [x] 인증 확인
  - [x] 좋아요 관계 확인
  - [x] 삭제 실행
  - [x] 에러 처리
- [x] 좋아요 상태 관리
  - [x] 클라이언트 상태 업데이트 (Optimistic Update)
  - [x] 좋아요 수 실시간 업데이트
  - [x] 에러 시 롤백
- [x] post_stats 뷰 post_id 필드 매핑 수정 (버그 수정)
...
- 로그인 트리거/화면 개선
  - 사이드바 하단 로그인 버튼을 Instagram 스타일 버튼으로 변경 (아이콘 제거, 파란 버튼)
  - 로그인 페이지 배경에 공개 피드 노출, 실패 시 스켈레톤 폴백
  - 배경 블러/오버레이로 글자 식별 불가 수준 처리, z-index 정렬
  - 로그인 페이지에 Header 표시
